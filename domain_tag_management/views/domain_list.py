...
class DomainListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            is_admin = role_code in ['admin', 'super_admin']
            ...
            if is_admin:
                ...
                # According to the business decision: Domains without a group are displayed to all users,
                # even if the user is not a member of any group associated with that domain.
                domains = Domain.objects.filter(
                    Q(group__in=user_group_ids) | Q(group__isnull=True),
                    deleted_at__isnull=True
                ).distinct()
